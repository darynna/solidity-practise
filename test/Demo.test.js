...
describe("Demo", function () {
  ...
  it("should allow to send money", async function() {
    ...
    await expect(() => sendMoneyTx)
      ...
      const timestamp =
          (await ethers.provider.getBlock(sendMoneyTx.blockNumber)).timestamp
    ...
    it("should allowed owner to withdraw money", async function () {
    ...
    it("should not allowes othe acounts withdraw money", async function () {
        const [_, amount] = await sendMoney(other_addr)

        await expect(demo.connect(other_addr).withdraw(other_addr.address)
